...
import FortranSrc.Repr.Value.Scalar.Logical.Machine
import FortranSrc.Repr.Value.Scalar.String
import FortranSrc.Repr.Type
import qualified FortranSrc.Repr.Eval.Op as Op
...
evalExpr = \case
  ...
  F.ExpFunctionCall _ _ ve args -> do
    let args' = case args of Nothing -> []; Just (F.AList _ _ as) -> as
    args'' <- traverse evalArg args'
    evalFunctionCall (forceVarExpr ve) args''
